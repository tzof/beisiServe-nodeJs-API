...
const { addOrder, getOrder, getOrderDetail, deleteOrder, updateOrder, getBackStock, getBackStockDetail, updateBackStock, getUserOrder, getDeliveryInfo,deleteOrderDetail } = require('../../modal/order')
...
router.post('/getOrderDetail', (req, res) => {
    ...
    let tDelivery = "select o.`code`,o.create_time,o.update_time,o.payment_type,o.`status` ,o.id,o.money,d.postid,d.ecp from t_order o join t_delivery d on o.id=d.order_id where d.order_id=?";
    let nDelivery = "select `code`,id,create_time,update_time,payment_type,money,`status` from t_order where id=?"
...
router.post('/getBackStock', (req, res) => {
    ...
    let sql = "select bs.id,bs.order_id,bs.create_time,bs.update_time,bs.status,bs.reason,bs.money from t_backstock as bs join t_order as o on o.id=bs.order_id WHERE o.store_id=9 and is_normal=0"
